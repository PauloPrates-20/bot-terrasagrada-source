...
const { firebaseApp } = require('./firebaseAppConfig');
...
async function registerPlayer(playerData) {
	...
	const ref = doc(db, userCollection, playerData.id.toString());
...
async function registerLog(logData, playerId) {
	const ref = collection(db, userCollection, playerId.toString(), 'logs');
	const convertedLog = {
		...logData,
...
async function loadPlayer(playerId) {
	const ref = doc(db, userCollection, playerId.toString());
